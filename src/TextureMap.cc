...
TextureMap::TextureMap(sf::Vector2u size) : size_(size)
{
    ...
    raw_data_ = std::shared_ptr<sf::Uint8>(new sf::Uint8[size.x * size.y * bpp],
                           ...
                           sf::Color color) noexcept
{
    ...
    std::memcpy(&raw_data_.get()[(x * bpp) + (y * size_.x * bpp)], &color_int,
                sizeof(color_int));
}

sf::Vector2u TextureMap::get_size() const noexcept { return size_; }

void TextureMap::update() noexcept { texture_.update(raw_data_.get()); }
